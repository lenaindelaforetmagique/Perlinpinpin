...
class Universe {
  constructor(order_ = 1) {
    ...
    this.width = 800;
    this.height = 800;
    this.landscape = new Landscape(this.width, this.height, 10);
    this.remesh = true;
    ...
    this.soundGun = new Audio('gun-gunshot-02.mp3');
  ...
  init() {
    ...
    var legend = "Utilisez les touches SDFE pour vous déplacer, haut/bas pour changer l'ordre de définition du sol.";
  ...
  mesh() {
    ...
    for (let i = 0; i < nbre; i++) {
      for (let j = 0; j < nbre; j++) {
        ...
        let z = 0; //this.landscape.altitude(x, y);
        this.addNode(x, y, z);
      }
    }
    ...
    while (tmpFaces.length > 0) {
      ...
      if (angle[0] > 40) {
        ...
        // if (angle[0] > 5 * DEG_TO_RAD) {
        // if (angle[0]) {
        let res = tmpFaces[0].refine(angle[1]);
        tmpFaces = tmpFaces.concat(res);
      } else {
        newFaces.push(tmpFaces[0]);
      }
      tmpFaces.splice(0, 1);
    ...
    for (let node of this.nodes) {
      node.position.z = this.landscape.altitude(node.position.x, node.position.y);
    }
  ...
  moveLong(intensity = 1) {
    this.camera.uz_speed = intensity;
    this.remesh = true;
  }

  moveLat(intensity = 1) {
    this.camera.ux_speed = intensity;
    this.remesh = true;
  }

  changeOrder(intensity = 1) {
    this.landscape.changeCurrentOrder(intensity);
    this.remesh = true;
  }
  ...
  addEvents() {
    ...
    this.container.addEventListener("mousedown", function(e) {
      e.preventDefault();
      console.log(thiz.soundGun);
      let newSound = new Audio("gunshot.mp3")
      newSound.play();
      // thiz.soundGun.play();
    }, false);
    ...
    document.addEventListener("mousemove", function(e) {
      ...
      if (e.buttons > 0) {
        ...
        thiz.camera.set_lat_speed(e.clientY / window.innerHeight - 0.5);
        thiz.camera.set_long_speed(e.clientX / window.innerWidth - 0.5);
      }
    }, false);
    ...
    this.container.addEventListener("mouseout", function(e) {
      e.preventDefault();
      thiz.camera.set_lat_speed(0);
      thiz.camera.set_long_speed(0);
    }, false);
